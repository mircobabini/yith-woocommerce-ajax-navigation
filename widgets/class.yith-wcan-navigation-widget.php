    ...
    class YITH_WCAN_Navigation_Widget extends WP_Widget {
        ...
        function widget( $args, $instance ) {
            ...
            if ( count( $terms ) > 0 ) {
                ...
                echo "</div>";
                ...
                } elseif ( $display_type == 'auto-color' ) {
                    // Auto Color display
                    echo "<ul class='yith-wcan-color yith-wcan yith-wcan-group'>";
                    ...
                    foreach ( $terms as $term ) {
						...
						$color = get_woocommerce_term_meta($term->term_id, 'pa_colors_yith_wccl_value', true);
						
                        echo '<li ' . $class . '>';

                        echo ( $count > 0 || $option_is_set ) ? '<a style="background-color:' . $color . ';" href="' . esc_url( apply_filters( 'woocommerce_layered_nav_link', $link ) ) . '">' : '<span>';
                        echo ( $count > 0 || $option_is_set ) ? '</a>' : '</span>';
        ...
        function form( $instance ) {
            ...
            <p><label for="<?php echo $this->get_field_id('type'); ?>"><strong><?php _e('Type:', 'yit') ?></strong></label>
                <select class="yith_wcan_type widefat" id="<?php echo esc_attr( $this->get_field_id('type') ); ?>" name="<?php echo esc_attr( $this->get_field_name('type') ); ?>">
                    ...
                    <option value="auto-color" <?php selected( 'auto-color', $instance['type'] ) ?>><?php _e( 'Auto Color', 'yit' ) ?></option>
